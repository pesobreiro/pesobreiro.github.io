	...
	public class categoria : System.Web.UI.Page
	{
		...
		private void CarregarEmpresas(string categoriaId)
		{
			...
			if (dt.Rows.Count == 0)
			{
				...
				c.Controls.Add(new LiteralControl("<p class='TituloTabela' align='center'>Não existem empresas nesta categoria!</p>"));
				...
				foreach ( DataRow dataRow in dt.Rows)
				{
					...
					businessDetail bd = empresa.ObterDetalhe(dataRow["EmpClass_Empresa_BusinessKey"].ToString());
				
					c.Controls.Add(new LiteralControl("<p class='TituloTabela'>" + bd.businessEntity[0].name[0].Value + "</p>"));
					c.Controls.Add(new LiteralControl("<p class='TextoNormal'><b>BusinessKey:</b>" + dataRow["EmpClass_Empresa_BusinessKey"] + "<br>"));
